...
import os
import math
from mss import mss #for screen recording
from PIL import Image #for screen recording

from classification import training, getLabel

SIGNS = ["ERROR",   #text to be display on top of the traffic sign when detected
        "STOP",
        "TURN LEFT AHEAD",
        "TURN RIGHT AHEAD",
        "DO NOT TURN LEFT",
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_sign_type):
    ...
    if sign is not None:
        ...
        sign_type = sign_type if sign_type <= 29 else 29
        text = SIGNS[sign_type]
        
        if sign_type > 0 and sign_type != current_sign_type: #display in extra window the traffic sign recognized       
          imS = cv2.resize(sign, (350, 350)) #size of the window
          cv2.imshow("Detected Sign", imS)  #display it and name of the display box
        
        cv2.imwrite(str(count)+'_'+text+'.png', sign)

    if sign_type > 0 and sign_type != current_sign_type:        
        cv2.rectangle(original_image, coordinate[0],coordinate[1], (0, 255, 0), 4) #change the 4 for the size of the rectangle
...
def main(args):
    ...
    #vidcap = cv2.VideoCapture(0) #for camera stream recording
    vidcap = cv2.VideoCapture(args.file_name) #for .mp4 file
	...
	#success,frame = vidcap.read()
        ...
        ##Screen recording happens and stored the frames into the constant frame. Remove everything after this and add line above and program will use either stream from camera or from an mp4 file##    
   
        #mon = {'top': 160, 'left': 160, 'width': 200, 'height': 200}

        sct = mss()

        monitor_number = 2 # here you write the monitor on which the traffic sign should operate
        ...
        if sign_type > 0 and (not current_sign or sign_type != current_sign):
            ...
            position = [count, sign_type if sign_type <= 29 else 29, coordinate[0][0], coordinate[0][1], coordinate[1][0], coordinate[1][1]]
            cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 4) #change the 4 for the size of the rectangle
            font = cv2.FONT_HERSHEY_PLAIN
            ...
            if sign_type > 0:
                ...
                position = [count, sign_type if sign_type <= 29 else 29, left, top, right, bottom]
                cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 4) #change the 4 for the size of the rectangle
                font = cv2.FONT_HERSHEY_PLAIN
                cv2.putText(image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
            elif current_sign:
                position = [count, sign_type if sign_type <= 29 else 29, tl[0], tl[1], br[0], br[1]]
                cv2.rectangle(image, (tl[0], tl[1]),(br[0], br[1]), (0, 255, 0), 4) #change the 4 for the size of the rectangle
                font = cv2.FONT_HERSHEY_PLAIN
